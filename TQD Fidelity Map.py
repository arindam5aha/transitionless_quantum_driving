...
kmin = 0                    # Kappa (Cavity 1 Damping Rate) (GHz)
kmax = 0.01
ym = 5*10**(-5)             # Mechanical Damping Rate (GHz)               
kqmin = 0                   # Qubit emission rate (GHz)
kqmax = 0.01
...
tlist = np.linspace(0,50,100)                        # Use range according to v

e_ops = [a.dag()*a, b.dag()*b, q.dag()*q]

#--------------------------------------
for k in np.arange(kmin, kmax, 0.0001):
    F_max = []                                      # Store F_max values for given k
    for kq in np.arange(kqmin, kqmax, 0.0001):
        # Collapse Operator
        #-------------------------------------
        cc = np.sqrt(k)*a
        ...
        F = []
        rho_m = []              
        S01 = tensor(basis(Nc,0),basis(Nq,1))                # State |a q> = |0 1>
        ...
        f_temp1 = np.real(np.array(list(map(lambda var: var.todense()[0][0], F))))
        f_temp2 = list(map(lambda var: var[0][0], f_temp1))

        F_max.append(np.amax(f_temp2))
    print(k*100/(kmax-kmin))
    zi.append(F_max)


xi = np.linspace(kmin,kmax,100)
yi = np.linspace(kqmin,kqmax,100)
...
plt.pcolor(x, y, zi, cmap = plt.get_cmap('rainbow'))
plt.colorbar() 
axes.set_xlabel(r'$\kappa$', fontsize=20)
axes.set_ylabel(r"$\Gamma$", fontsize=20)
plt.show()